...
O ponto que permanece em aberto é a recriação e execução bem-sucedida do `system_check.sh` no cenário de auditoria de processos, para confirmar se o evento correspondente aparece corretamente no Wazuh. Somente depois disso será avaliada a necessidade de uma regra customizada de detecção para o evento `soc_lab_exec`, evitando mascarar um eventual problema de coleta com uma regra criada prematuramente.
